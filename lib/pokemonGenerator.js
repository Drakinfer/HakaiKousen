...
export function getRandomNature() {
  const keys = Object.keys(NATURES); // [ "Hardi", "Solo", "Rigide", ... ]
  const randomIndex = Math.floor(Math.random() * keys.length); // 0 → 24
  return keys[randomIndex];
}

export function getRandomSubNature() {
  const randomIndex = Math.floor(Math.random() * SUB_NATURES.length); // 0 → 99
...
const getRandomIV = () => Math.floor(Math.random() * 4); // 0–3
